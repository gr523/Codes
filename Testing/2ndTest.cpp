...
#define BYTETOBINARY(byte)  \
  (CryptoPP::byte & 0x80 ? 1 : 0), \
  (CryptoPP::byte & 0x40 ? 1 : 0), \
  (CryptoPP::byte & 0x20 ? 1 : 0), \
  (CryptoPP::byte & 0x10 ? 1 : 0), \
  (CryptoPP::byte & 0x08 ? 1 : 0), \
  (CryptoPP::byte & 0x04 ? 1 : 0), \
  (CryptoPP::byte & 0x02 ? 1 : 0), \
  (CryptoPP::byte & 0x01 ? 1 : 0) 
...
string PBKDF2_HMAC_SHA_512_string(string pass, string salt, uint iterations, uint outputBytes)
{
  SecByteBlock result(outputBytes);
	...
	pbkdf.DeriveKey(result, result.size(),0x00,(CryptoPP::byte *) pass.data(), pass.size(),(CryptoPP::byte *) salt.data(), salt.size(),iterations);
...
int main(int argc, char ** argv)
{
  ...
  CryptoPP::byte verbose = 0, help = 0, interactive = 0;
  std::locale loc;
